...
- `r-cadence` places a definitional cadence (`PAC`, `IAC`, `HC`/`half`, `deceptive`, `plagal`) at a two-position pair, requiring a key declared in `harmonic_domain` at both. Deliberately thin: bass motion / end-degree are validated once against the harmony plan at compile time (not a Gecode constraint), while root position and soprano target degree are posted as constraints. General voice leading (parallel fifths/octaves, spacing) stays in `r-pitch-pitch` since it applies everywhere, not just at cadences. `metric_strength` and leading-tone/chordal-7th resolution are accepted/logged but not yet enforced — see [USAGE.md §5.3d](USAGE.md#53d-cadence-rule-r-cadence).
- `r-repetition` relates a source position range to a target range within one or more voices: `exact`, `transposed` (fixed semitone offset), `contour_preserving` (matching up/down/same direction per step), `rhythm_preserving` (rhythm matches, pitch free), or `diatonic_sequence` (shift by scale _steps_, not semitones, within a shared key — posted as an exact Gecode `extensional` table constraint). Supports `"heuristic": true` for the first three relations; per the design doc, phrase/motivic relations default to soft while cadences default to hard. See [USAGE.md §5.3e](USAGE.md#53e-repetition--sequence-rule-r-repetition).
